...
LastEditTime: 2021-01-03 18:58:32
...
class weatherInfo(object):
    ...
    # 运行播报今日天气预报的程序
    @staticmethod
    def run_today_weather():
        # 获取今日天气预报信息
        weather_data = weatherInfo.Get_weather_data()
        weather_forecast_txt = weatherInfo.Show_weather(weather_data)
        # 语音播报今日天气预报信息, 最后将其文本数据推送到微信
        ttsToool.Voice_broadcast(forcast_txt=weather_forecast_txt)

    # 运行播报明日天气预报的程序
    @staticmethod
    def run_tomorrow_weather():
        # 获取明日天气预报信息
        weather_data = weatherInfo.Get_weather_data()
        weather_forecast_tomorrow_txt = weatherInfo.Show_weather_tomorrow(
            weather_data)
        # 语音播报明日天气预报信息, 最后将其文本数据推送到微信
        ttsToool.Voice_broadcast(forcast_txt=weather_forecast_tomorrow_txt)
