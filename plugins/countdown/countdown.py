...
LastEditTime: 2021-01-03 16:55:13
Description: 倒计时
...
class countdown(object):
    ...
    def inputDate(input_years, input_month, input_day):
        ...
        if input_years < localtime[0]:
            print("请输入大于或等于当前年份")
        # 检查用户输入的月份及日是否合理
        if (countdown.isRight(input_years, input_month, input_day)):
            # 查询的年份距离当前年份是同一年
            if input_years == tm_year:
                forcast_txt = countdown.sameYear(input_years,
                                                 input_month, input_day)
                return forcast_txt
            # 查询的年份距离当前年份大于一整年
            elif input_years - tm_year == 1:
                forcast_txt = countdown.nextYear(input_years,
                                                 input_month, input_day)
                return forcast_txt
            # 查询的年份距离当前年份大于一整年以上
            else:
                forcast_txt = countdown.diffYear(input_years,
                                                 input_month, input_day)
                return forcast_txt
        else:
            print("你输入的日期数据不合理 !")
        ...
        for i in range(1, input_month + 1):
            for j in range(1, 32):
                ...
                if i in [1, 3, 5, 7, 8, 10, 12]:
                    ...
                    if countdown.isLeap(input_years):
                        if j >= 29:
                            break
                    else:
                        if j >= 28:
    ...
    def sameYear(input_years, input_month, input_day):
        days = countdown.getDays(input_years, input_month, input_day)
        ...
        print("\r距离%d年%d月%d日考研初试还剩%d天%02d小时%02d分钟%02d秒 !" %
              (input_years, input_month, input_day, days - tm_y_day - 1,
               Var_Hour, Var_Min, Var_Sec),
              end="")
        forcast_txt = \
             "距离%d年%d月%d日考研初试还剩%d天%02d小时%02d分钟%02d秒 !" % \
              (
                input_years,
                input_month,
                input_day,
    ...
    def nextYear(input_years, input_month, input_day):
        ...
        print("\r距离%d年%d月%d日还剩%d天%02d小时%02d分钟%02d秒 !" %
              (input_years, input_month, input_day, d, Var_Hour, Var_Min,
               Var_Sec),
              end="")
        forcast_txt = \
             "距离%d年%d月%d日考研初试还剩%d天%02d小时%02d分钟%02d秒 !" % \
    ...
    @staticmethod
    def run():    
        # 2022 考研初试时间为 2021 12 26
        string = countdown.inputDate(input_years=2021,
                                    input_month=12,
                                    input_day=26)
        # 语音播报距离 2021 年考研初试的倒计时, 最后将其文本数据推送到微信
        ttsToool.Voice_broadcast(forcast_txt=string)
